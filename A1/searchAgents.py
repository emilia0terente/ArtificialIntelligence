...
def cornersHeuristic(state, problem):
    ...
    prev_pos, visited_corners = state[0], state[1]

    unvisited_corners = [corner for corner in corners if corner not in visited_corners]

    heuristic = sum(util.manhattanDistance(prev_pos, corner) for corner in unvisited_corners)
...
#DijkstraSearch
class DijkstraSearchAgent(SearchAgent):
    def __init__(self):
        self.searchFunction = search.dijkstraSearch
        self.searchType = PositionSearchProblem

#A*Weighted
class AStarWeightedCornersAgent(SearchAgent):
    "A SearchAgent for CornersProblem using A* Weighted and cornersHeuristic"
    def __init__(self, weight=1):
        self.searchFunction = lambda prob: search.aStarWeightedSearch(prob, cornersHeuristic, weight)
        self.searchType = CornersProblem

#Incremental heuristic search
class IncrementalHeuristicSearchAgent(SearchAgent):
    def __init__(self, timeLimit=1):
        self.searchFunction = lambda prob: search.incrementalHeuristicSearch(prob, cornersHeuristic, timeLimit)
        self.searchType = CornersProblem
